      ...
      char **argv)
{
  ...
  if (!opt_finish_only && g_file_query_exists (app_dir, NULL) && !directory_is_empty (app_dir_path))
    {
      if (opt_force_clean)
        {
  ...
  if (!opt_finish_only && !builder_cache_lookup (cache, "init"))
    {
  ...
  if (!opt_finish_only
      && !builder_manifest_build (manifest, cache, build_context, &error))
    {
      g_printerr ("Error: %s\n", error->message);
      return 1;
    }
