      ...
      if (collection_id != NULL && *collection_id != '\0')
        {
          ...
          rev = g_hash_table_lookup (results[0]->ref_to_checksum, &collection_ref);
        }
      else
#endif  /* FLATPAK_ENABLE_P2P */
        {
                                  ...
                                  GError             **error)
{
  ...
  if (gpg_verify)
    {
      gpg_result = ostree_repo_verify_commit_ext (src_repo, checksum, NULL, NULL, cancellable, error);
      if (gpg_result == NULL)
        return FALSE;
                              ...
                              GError             **error)
{
  ...
  if (no_pull)
  ...
  else if (collection_id != NULL)
    {
      ...
      latest_rev = g_strdup (g_hash_table_lookup (results[0]->ref_to_checksum, &collection_ref));
      if (latest_rev == NULL)
        return NULL;
                             ...
                             GKeyFile     *new_config)
{
  ...
  if (gpg_verify && collection_id == NULL)
    {
